...
@Component({
  ...
  imports: [FormsModule, CommonModule, DragDropModule, CheckboxModule],
  templateUrl: './product-tables.component.html',
  styleUrls: ['./product-tables.component.scss'],
})
export class ProductTablesComponent implements OnInit {
  ...
  set productsInput(value: IProduct[]) {
  ...
  openContextMenu(event: MouseEvent, column: ColumnsConfig): void {
    ...
    this.position.update((pos) => ({ ...pos, x: event.clientX }));
    this.position.update((pos) => ({ ...pos, y: event.clientY }));
  ...
  isCustomerArray(items: IProduct[] | GroupedData[]): items is IProduct[] {
  ...
  selectTableData(event: any, customer: IProduct): void {
    const isSelected = event.checked;
    const updatedProducts = this.products().map((prod) =>
      prod.id === customer.id ? { ...prod, selected: isSelected } : prod,
    );
